...
import br.UFSC.GRIMA.capp.movimentacoes.estrategias.FreeFormStrategy;
import br.UFSC.GRIMA.capp.movimentacoes.estrategias.MachiningStrategy;
...
//		}
		...
		if(zMaximo>regionTmp.getPosicaoZ()){
			...
			desbaste1.setMachiningStrategy(machiningStrategy);

			//---------criando plunge
			PlungeToolAxis plunge = new PlungeToolAxis();
			plunge.setToolDirection(new Point3d(0, 0, -1));
			desbaste1.setApproachStrategy(plunge);
			...
			machiningStrategy.setOverLap(faceMill.getDiametroFerramenta() * 0.25);
		...
		BottomAndSideRoughMilling desbaste1 = new BottomAndSideRoughMilling("Bottom And Side Rough Milling", retractPlane);
					...
					// ---- Cirando estategia
		ContourParallel machiningStrategy = new ContourParallel();
...
//		}
		...
		machiningStrategy.setOverLap(endMill.getDiametroFerramenta() * 0.25);
		machiningStrategy.setRotationDirection(RotationDirection.CCW);
...
//		System.err.println("================>>>>>>>> COND US = " + condicoesDeUsinagem.getAp());
		...
		FreeFormStrategy freeFormStrategy = new FreeFormStrategy();
		freeForm.setMachiningStrategy(freeFormStrategy);
		//---------criando plunge
		PlungeToolAxis plunge1 = new PlungeToolAxis();
		plunge1.setToolDirection(new Point3d(0, 0, -1));
		freeForm.setApproachStrategy(plunge1);
			// -------- Criando Ferramenta --------

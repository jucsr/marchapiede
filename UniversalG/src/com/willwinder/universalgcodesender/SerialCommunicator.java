...
public class SerialCommunicator implements SerialPortEventListener{
    ...
    */
    ...
    /**
     * by Jc -- Setup a NC program for streaming to serial port then launch the first command
     */
    void streamFileToComm(String gCodeText) 
    {
        // Get command list.
    		String [] linhas = gCodeText.split("\n");
        	
    		GcodeCommand command;
    		for(int i = 0; i < linhas.length; i++)
    		{
    			String lineTmp = linhas[i];
                // Add command to queue
                command = this.commandBuffer.appendCommandString(lineTmp + '\n');

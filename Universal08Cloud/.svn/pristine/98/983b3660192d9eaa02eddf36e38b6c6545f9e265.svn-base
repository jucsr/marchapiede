...
public class EditFaceMillWS extends EditFaceMillFrame implements ActionListener, ItemListener, ChangeListener
{
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{
			this.label2.setIcon(new ImageIcon(getClass().getResource("/images/defaultStrategie.png")));
			ContourParallel strategy = new ContourParallel();
			
			this.operation.setMachiningStrategy(strategy);
		}
		else if(object == radioButton2)
		{
			this.label2.setIcon(new ImageIcon(getClass().getResource("/images/trochoidalAndContourParallelStrategie.png")));
...
//		    machiningStrategyTmp.setOverLap(0.25*faceMillTmp.getDiametroFerramenta()); //Overlap
			double trochoidalRadius = ((Double)this.spinner16.getValue()).doubleValue();
			strategy.setOverLap(2); //Overlap
			strategy.setTrochoidalRadius(trochoidalRadius); //REVER MAIS TARDE
			strategy.setTrochoidalSense(TrochoidalAndContourParallelStrategy.CCW);
			strategy.setCutmodeType(TrochoidalAndContourParallelStrategy.conventional);
			this.operation.setMachiningStrategy(strategy);
		}
	}
	private void ok() 
